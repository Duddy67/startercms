...
class AdminUsersRoles
{
	...
	if (in_array($routeName, $update) && !auth()->user()->isAllowedTo('update-role')) {
	    return redirect()->route('admin.users.roles.index')->with('error', __('messages.roles.edit_not_auth'));
	}

	if (in_array($routeName, $delete) && !auth()->user()->isAllowedTo('delete-role')) {
	    return redirect()->route('admin.users.roles.index')->with('error', __('messages.roles.delete_not_auth'));
	}
